...
#[allow(dead_code)]
const SEEN_SET_FILENAME: &str = ".rss_seen_guids.ron";

/// Default path for persistent RSS seen GUIDs.
#[allow(dead_code)]
pub(crate) fn default_seen_set_path() -> PathBuf {
    PathBuf::from(SEEN_SET_FILENAME)
}
...
mod tests {
    ...
    #[allow(dead_code)]
    fn temp_file_path(temp: &TempDir, name: &str) -> PathBuf {
        temp.path().join(name)
    }
    ...
    fn save_and_load_roundtrips() {
        ...
        save_seen_set(&set, &path).expect("save");
    ...
    fn save_failure_reports_error_when_parent_not_dir() {
        ...
        let err = save_seen_set(&set, &path).unwrap_err();
