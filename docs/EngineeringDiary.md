...
Change: Updated `harvester_app` Prompt Lab layout to compute visibility from a canonical predicate set and use the same `show_template_editor_rows` condition for both panel height and row sizing. Added reducer-side state normalization in `harvester_core::PromptLabState` so `template_editor_open` cannot remain true when Prompt Lab is hidden, advanced mode is disabled, or the template section is closed.
Evidence: `cargo test -p harvester_app platform::ui::layout::tests::template_editor_rows_are_collapsed_when_editor_is_closed -- --nocapture`; `cargo test -p harvester_app platform::ui::layout::tests::template_toggle_button_width_fits_state_text -- --nocapture`; `cargo test -p harvester_app platform::ui::layout::tests::template_editor_visibility_matrix_enforces_row_sizes -- --nocapture`; `cargo test -p harvester_core close_resets_template_editor_open_invariant -- --nocapture`; `cargo test -p harvester_core disabling_advanced_mode_closes_template_editor -- --nocapture`; `cargo test -p harvester_core closing_template_section_closes_template_editor -- --nocapture`; `cargo build`; `cargo clippy --all-targets -- -D warnings`.
