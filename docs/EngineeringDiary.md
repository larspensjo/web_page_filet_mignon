...
## 2026-03-10 - Resize and splitter drags no longer stall on large job trees
Type: Bug Fix
Context: Dragging the main window border or pane splitter became nearly unresponsive after the TreeView selection-styling update, especially with a populated output folder. The investigation needed to separate app-side geometry batching from native Win32 repaint cost and identify whether the regression was event delivery, view rebuilding, or pathological control paint behavior.
Change: Updated `harvester_app` so geometry-only batches use a lightweight layout render path and emit `[ui-geometry]` timing logs, then iteratively hardened `commanductui` by trimming TreeView custom-draw work, removing synchronous layout repaint calls, adding native dispatch/layout diagnostics, and finally suspending TreeView redraw during live sizing/splitter drags before issuing a single repaint on release. The final evidence isolated the remaining stall to slow TreeView `WM_PAINT` work under width changes, so the accepted mitigation lives in the `commanductui` resize/drag boundaries. The submodule was advanced through `0.8.6` and the workspace lockfile was refreshed.
Evidence: `cargo test` in `src/CommanDuctUI`; `cargo test -p harvester_app`; `cargo build`; `cargo clippy --all-targets -- -D warnings`; `engine.log` showed `[ui-geometry]` batches staying near 20-25 ms while native timing logs narrowed the remaining pause to slow TreeView `WM_PAINT` and confirmed smooth resize once TreeView redraw was suspended during drag.
Lessons Learned: Interactive geometry changes are a cross-layer performance contract. Fixing reducer/render costs is necessary but not sufficient when a Win32 control can still dominate latency inside the modal sizing loop; diagnostics must cover app batching, native layout, dispatch, and control paint to localize the real bottleneck.
Prevention: Split geometry rendering from data rendering by default, ban synchronous repaint calls from hot resize/layout loops unless justified, and define an explicit live-drag repaint policy for heavy custom-drawn controls so regressions are caught before a feature ships.
Refs: crates/harvester_core/src/view_model.rs, crates/harvester_core/src/state.rs, crates/harvester_app/src/platform/app.rs, crates/harvester_app/src/platform/ui/render.rs, src/CommanDuctUI/src/controls/treeview_handler.rs, src/CommanDuctUI/src/window_common.rs, src/CommanDuctUI/src/app.rs, src/CommanDuctUI/Cargo.toml, src/CommanDuctUI/CHANGELOG.md
