...
- **Step 3 → Step 9 allow-list replacement**: the interim allow-list built from `config` models and `PricingRegistry` keys should be replaced with the formal model catalog introduced in Step 9, switching to a keyed-by-`ModelId` lookup that drops the current two-step string/provider check.
- **Per-run `temperature` and `max_tokens` overrides**: add alongside the existing `model_override` using the same `validate_model_override` pattern. The helper generalises naturally into a `validate_run_overrides` struct; add both fields to `LlmCommand::Complete` and `Effect::RequestLlmCompletion` following the same None-means-default convention.
- **Prompt Lab UI override controls (Step 4)**: surface `selected_model_override` and `selected_prompt_version` in the Prompt Lab panel once Step 4 adds the UI layer. Persist the last-used override per stage as a convenience default (stored in `PromptLabState`, not in `LlmConfig`).
- **Cross-provider override (post-catalog)**: once the Step 9 catalog marks models as functionally equivalent across providers, allow overriding with a model from a different provider than the configured one — requires provider-aware routing and is explicitly out of scope until the catalog exists.
- **Run record export for comparison**: export `prompt_version_used`, `model_override`, and full `LlmRunMetadata` from `PromptLabRunRecord` as structured JSON for side-by-side offline comparison. Natural extension of Step 9's export path.
